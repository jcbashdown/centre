class NodesController < ApplicationController
  ...
  def set_argument
    if current_user
      positive = GlobalNodeUser.where(:node_id => @node.id, :global_id =>  @global.id, :user_id => @user.id)[0].positive_node_argument
      if positive && positive.content.length > 0
        @positive_argument = positive.friendly_argument
      end
      negative = GlobalNodeUser.where(:node_id => @node.id, :global_id =>  @global.id, :user_id => @user.id)[0].negative_node_argument
      if negative && negative.content.length > 0
        @negative_argument = negative.friendly_argument
      end
      p @negative_argument
      p @positive_argument
    else
