class GlobalNodeUser < ActiveRecord::Base
  ...
  def node_hash
    {:title => self.title, :body => self.body, :is_conclusion => self.is_conclusion}
  end
  ...
  def set_or_create_node_user
    nu = NodeUser.where(:user_id => self.user_id, :node_id => self.node_id, :is_conclusion => self.is_conclusion)[0] || NodeUser.create!({:user_id => self.user_id, :node_id => self.node_id}.merge(node_hash))
    self.node_user = nu
    save
  end
