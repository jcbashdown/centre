...
describe LinksController do
  ...
  describe 'update' do
    before do
      ...
      @user_link = Link::UserLink::PositiveUserLink.create(@params_one["global_link"].merge(:question => @question, :user => @user))
      @user_two = FactoryGirl.create(:user, :email => "user2@test.com")
      @user_linkuser2 = Link::UserLink::PositiveUserLink.create(@params_one["global_link"].merge(:question => @question, :user => @user_two))
    end
    ...
    context 'when ajax request' do
      context 'with valid params' do
        before do
          @params = {"type" => "Negative", "global_link"=>{"global_node_from_id"=>@node_one.id.to_s, "global_node_to_id"=>@node_two.id.to_s}, "id" =>@user_link.global_link.id, "view_configuration" => {"links_from_question" => @question.id}}
          @mock_cn = mock('user_link')
          @mock_cn_2 = mock('user_link')
        ...
        it 'should save the glu' do
          Link::UserLink::NegativeUserLink.stub(:find).and_return @mock_cn
          Link::UserLink::NegativeUserLink.stub(:new).and_return @mock_cn_2
      ...
      context 'when destroy or save returns false for glu' do
        before do
          ...
          Link::UserLink::NegativeUserLink.stub(:create).and_return false
        end
  ...
  describe 'create' do
    ...
    context 'when ajax request' do
      context 'with valid params' do
        ...
        it 'initialise a new link with the correct parameters' do
          Link::UserLink::NegativeUserLink.should_receive(:new).with(@params["global_link"].merge("question_id" => @question.id, "user" => @user)).and_return @mock_cl
          xhr :post, :create, @params
        end
      end
      context 'when save returns false for glu' do
        ...
        it 'initialise a new link with the correct parameters' do
          Link::UserLink::NegativeUserLink.should_receive(:new).and_return @mock_cl
  ...
  describe 'destroy' do
    before do
      ...
      @user_two = FactoryGirl.create(:user, :email => "user2@test.com")
      @user_linkuser2 = Link::UserLink::PositiveUserLink.create!(@params_one["global_link"].merge(:question => @question, :user => @user_two))
      @params_one = {"type" => "Positive", "global_link"=>{"global_node_from_id"=>@node_one.id.to_s, "global_node_to_id"=>@node_two.id.to_s}, :id => @user_link.global_link.id, :view_configuration => {:links_from_question => @question.id}}
      @user_link.global_link.should == @user_linkuser2.global_link
    end
    #update will destroy and then create as too much logic for an after save if we check if we need to to destroy question links etc.
    context 'when ajax request' do
      context 'with valid params' do
        ...
        it 'should save the glu' do
          Link::UserLink.stub(:where).and_return [@mock_cl]
        ...
        it 'should assign the correct link' do
          @new_link_params = {"global_link"=>{"global_node_from_id"=>@node_one.id.to_s, "global_node_to_id"=>@node_two.id.to_s}}
          new_link = Link::UserLink::PositiveUserLink.new(@new_link_params["global_link"])
      ...
      context 'when destroy or save returns false for glu' do
        before do
          @user_link.stub(:destroy).and_return false
          Link::UserLink::PositiveUserLink.stub(:find).and_return @user_link
        end
        it 'should render the link template' do
          xhr :put, :destroy, @params_one
          response.should render_template(:partial => "_a_link")
