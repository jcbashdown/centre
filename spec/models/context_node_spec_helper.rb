...
shared_examples_for "a node deleting nodes correctly" do
  ...
  it 'should update the global node caches' do
    ...
    gn = n.global_node
    n.destroy
    gn.reload.users_count.should == @state_hash[:global_node][:users_count] unless (@state_hash[:global_node][:destroyed] < 0)
  end

  it "should delete user links if the user node was deleted" do
    if @state_hash[:user_node][:destroyed] < 0
      Link::UserLink.should_receive(:where).with('user_id = ? AND (global_node_from_id = ? || global_node_to_id = ?)', @node.user_id, @node.global_node_id, @node.global_node_id).and_return(mock_relation = mock('relation'))
